    ...
    <div class="flex-col px-4 py-5 space-y-4 bg-white sm:p-6">
        <header>
            <h3 class="text-lg font-medium leading-6 text-gray-900">
                {{ __('image_manager.heading') }} 
                <small class="text-gray-400 max-w-xs block overflow-hidden whitespace-nowrap">
                    {{ $attributes->get('subtitle') ?? $attributes->get('subtitle') }}
                </small>
            </h3>
        </header>

        <div>
            <x-admin.components.input.fileupload label="<span class='plus text-gray-400'>+</span>" wire:model="{{ $wireModel }}" :images="$images" :filetypes="$filetypes" :maxFiles="$maxFiles" :maxFileSize="$maxFileSize" :multiple="$multiple" />
        </div>

        @if ($errors->has($wireModel . '*'))
            <x-alert level="danger">{{ __('image_manager.generic_upload_error') }}</x-alert>
        @endif
                    ...
                    <div
                        ...
                        <div class="flex items-center w-full space-x-6">
                            ...
                            <div class="flex items-center ml-4 space-x-4">
                                ...
                                @if (!empty($image['id']))
                                    <x-admin.components.tooltip :text="__('image_manager.remake_transforms')">
